                        ...
                        <div class="p-6 bg-white flex justify-center relative overflow-hidden h-64 border-b border-gray-100 dark:border-gray-700">
                            ...
                            @if($product->image_blob)
                                <img src="{{ $product->image_blob }}" 
                                     alt="{{ $product->name }}" 
                                     class="h-full w-full object-contain transition-transform group-hover:scale-110 duration-300 {{ $product->stock == 0 ? 'opacity-50 grayscale' : '' }}">
                            ...
                            {{-- ⭐ NUEVO: ETIQUETAS FLOTANTES DE STOCK ⭐ --}}
                            <div class="absolute top-2 right-2 flex flex-col gap-1 items-end z-10">
                                @if($product->stock == 0)
                                    <span class="bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow-md">
                                        🚫 AGOTADO
                                    </span>
                                @elseif($product->stock <= 5)
                                    <span class="bg-yellow-400 text-black text-xs font-bold px-2 py-1 rounded-full shadow-md animate-pulse">
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                                ...
                                @if(Auth::check() && Auth::user()->role === 'admin')
                                    {{-- SI ES ADMIN (Siempre puede editar, aunque no haya stock) --}}
                                ...
                                @else
                                    {{-- SI ES CLIENTE (Lógica de Stock) --}}
                                    @if($product->stock > 0)
                                        {{-- Hay stock: Botón Azul --}}
                                    ...
                                    @else
                                        {{-- No hay stock: Botón Gris Bloqueado --}}
                                        <button disabled 
                                           class="flex items-center gap-2 bg-gray-400 cursor-not-allowed text-white px-4 py-2 rounded-lg text-sm font-bold shadow-none">
                                            🚫 Agotado
                                        </button>
                                    @endif
                                @endif
